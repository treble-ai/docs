---
title: "Bienvenido a Treble"
description: "Treble es una herramienta que te permite crear flujos de conversación personalizados para tus casos de uso de ventas y marketing en WhatsApp. Adicionalmente, Treble se integra con tus sistemas de información como HubSpot y Salesforce para automatizar y optimizar tus procesos de mensajería por WhatsApp."
sidebarTitle: "Descripción General"
---

<img src="/images/treble-platform.png" alt="Treble Plataforma" />

## Primeros Pasos

Si eres nuevo en Treble, comienza aquí para aprender lo esencial y poder crear tus primeras conversaciones en WhatsApp.

<CardGroup cols={3}>
    <Card 
        title="Introducción a Treble" 
        icon="check"
        href="/es/docs/intro-to-treble/welcome"
    >
        Crea y envía tu primera conversación outbound en Treble.
    </Card>
    <Card 
        title="Referencia de la API" 
        icon="code"
        href="https://treble.notion.site/Treble-API-Documentation-b0a726d95018471f88e30a13db6f87b4?pvs=4"
    >
        Utiliza nuestra API para integrar Treble con tus sistemas de información.
    </Card>
    <Card 
        title="Integraciones" 
        icon="circle-nodes"
        href="/es/docs/integrations/welcome"
    >
        Descubre como puedes integrar Treble con tu CRM u otros sistemas de información.
    </Card>
</CardGroup>

<div class="hr"/>